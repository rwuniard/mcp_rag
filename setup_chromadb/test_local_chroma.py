                ...
                while offset < count:
                    ...
                    if batch['documents']:
                        for i, doc in enumerate(batch['documents']):
                            metadata = batch['metadatas'][i] if batch['metadatas'] and i < len(batch['metadatas']) else {}
...
def check_data_directory():
    ...
    print(f"\n{'='*60}")
    print(f"📂 Checking Data Source Directory")
    print(f"{'='*60}")
    ...
    if not data_source_dir.exists():
        print(f"❌ Data source directory not found")
        return
    
    # List all files
    files = list(data_source_dir.glob("*"))
    if not files:
        print(f"📭 No files found in data source directory")
        return
...
def main():
    ...
    google_total, google_word, google_types = analyze_database_safe(google_db_path, "Google Embeddings")
    
    # Analyze OpenAI embeddings database
    openai_db_path = data_dir / "chroma_db_openai"
    openai_total, openai_word, openai_types = analyze_database_safe(openai_db_path, "OpenAI Embeddings")
    ...
    if total_word_docs == 0 and word_files:
        print(f"\n⚠️  ISSUE DETECTED:")
        print(f"   📝 Word files are available but not stored in ChromaDB")
        print(f"   💡 Try running the storage process:")
        print(f"      python main.py store")
        print(f"   💡 Or run directly:")
        print(f"      python src/rag_store/store_embeddings.py")
    elif total_word_docs > 0:
        print(f"\n✅ SUCCESS:")
        print(f"   📝 Word documents are successfully stored in ChromaDB!")
        print(f"   🔍 You can now search them using:")
        print(f"      python main.py search \"your query about word content\"")
    elif not word_files and total_word_docs == 0:
        print(f"\n📝 TO ADD WORD DOCUMENTS:")
        print(f"   1. Place .docx or .doc files in: src/rag_store/data_source/")
        print(f"   2. Run: python main.py store")
        print(f"   3. Search: python main.py search \"your query\"")
